...
VITE_BASE=/life-swipe/ npm run build   # the GitHub Pages build (see "Deployed
                                       # to GitHub Pages"). Plain `npm run
                                       # build` keeps base "/", which is what
                                       # `npm start` needs.
```
...
<https://kevinraffay.github.io/life-swipe/?demo=1> — the whole player client
on a static host, with demo mode reached through the kiosk link it already
had. `.github/workflows/pages.yml` builds and publishes it on every push to
`main`.
...
- **Two 404s on load are expected and correct.** `getConfig()` and
  `fetchRegion()` fire at boot and are caught — the game reads a missing
  config as `llmEnabled: false` and shows "Storyteller offline", which is the
  truth on a static host, and a missing region as no regional weighting. Note
  they resolve to the github.io **root** rather than the project path, since
  `client/src/api.js` uses absolute `/api/...`. Harmless; not worth a
  build-time flag unless the console noise starts mattering.

The bare URL (without `?demo=1`) still offers a full life. On Pages that runs
on the seed deck alone and says so. That is honest rather than ideal — a
static-demo build flag was considered and deliberately not built, since the
URL already does the job.
