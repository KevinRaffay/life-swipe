...
| `client/src/components/Hud.jsx` | the stats HUD (money/health/happiness/age) and the shared money formatter. Takes an optional `swipeCap` (demo mode only) that turns the counter into "swipe 12 of 32". |
...
`BAL.DEMO.maxSwipes` (32) is a ceiling, not a target. Bankruptcy and death end
...
thirty-second swipe ends as broke, not as "the demo finished".
...
The HUD shows `swipe 12 of 32` in demo mode. That one string is what stops the
...
meaningful chance of natural death inside thirty-odd swipes needs roughly 1.5
...
ordinary rate a 32-swipe demo covers under three years and ends at 20, which
...
the age the demo reaches. When measurement pulled `maxSwipes` down from 40 to
32, `time.minor` had to go 4 → 5 with it: 32 × 4 months ends the demo at 28.7,
which never enters the pool's third age band and would strand its ~180 cards
as unreachable. 32 × 5 ends at 31.3, which does. Change one and re-check the
other, and read the age line `npm run demo-check` prints.
