...
| `client/src/prefs.js` | per-player cross-life memory in `localStorage` (content mode, age gate, region choice, theme choice, `seen_patterns`/`seen_seed_ids`), every access wrapped. Theme functions: `getTheme()` (the stored choice or null), `setTheme()`, `getActiveTheme()` (the choice, else **dark** - not the OS preference), and `applyTheme()`, the single place `.dark-theme` is put on the document. |
| `client/src/staticDemo.js` | one build-time constant, `STATIC_DEMO` (`VITE_STATIC_DEMO=1`), true only on the deployed GitHub Pages build. Decides which entry point the start screen offers; never reaches `state.demoMode`, the engine or the deck. |
...
| `client/src/components/CardStack.jsx` | the swipe gesture (pointer events), tiered card rendering, `renderedTier` (which tier a card actually renders as, read off the fields present rather than off `weight`, for the height class), and `useFitToCard` — the measure-and-shrink pass that keeps a long card inside its box so it never scrolls. |
| `client/src/components/Hud.jsx` | the stats HUD (money/health/happiness/age) and the shared money formatter. Takes an optional `swipeCap` (demo mode only) that turns the counter into "swipe 12 of 30". |
| `client/src/components/StartScreen.jsx` | the start screen: content-mode pick, age confirmation, region choice, theme toggle (light/dark, two states), and the separate demo entry link below the Begin button. `askingAge` is `null`/`'mature'`/`'demo'` - one gate dialog, two callers, since demo mode is mature content and goes through it rather than around it. Takes `staticDemo`: on the demo-only build Begin starts the demo and the mode picker and demo link are not rendered. |
...
VITE_BASE=/life-swipe/ VITE_STATIC_DEMO=1 npm run build   # the Pages build:
                                       # subpath assets, demo-only start
                                       # screen. Plain `npm run build` keeps
                                       # base "/" and the full game, which is
                                       # what `npm start` needs. (In Git Bash,
                                       # MSYS rewrites the /life-swipe/ value
                                       # into a Windows path - use PowerShell
...
| Minor cards take their own height | shipped | on `github-pages-demo` — a prompt-only card was being stretched to the height a four-field major needs, which put one short paragraph in the middle of an empty white slab, and left the choice buttons crowding the card's bottom edge at the 12px `.app` gap. `renderedTier(card)` (`CardStack.jsx`) now stamps `card--minor`/`card--standard`/`card--major` on both the top and peek cards, read off WHICH FIELDS ARE PRESENT rather than off `card.weight` — the two agree in the content files, but the layout cares about what is on screen, and a card whose beat failed validation is a bare prompt however it was labelled. `.card--minor` gets `height: fit-content` with a `min-height: 220px` floor, still centred by the existing `inset: 0`/`margin: auto`, so the space that used to sit INSIDE the card now sits around it and the buttons gain room without moving. The inherited `max-height` is unchanged, so a long minor prompt still grows to the same ceiling and then lets `useFitToCard` shrink the type — the behaviour that was already there. Measured on the deployed demo at 744px viewport: card 370px → 220-277px depending on content, gap above the buttons 12px → ~80px, `--fit` stays 1 across ten consecutive cards (no type shrinking) and nothing clips. Standard and major cards are untouched, PROVEN by removing the class from a live card and watching it return to exactly 370px, not by trusting the selector. Applies to every minor card, not only the demo's — which includes the intro's two identity choices, checked in the full build and improved by the same reasoning. Presentation only: no engine, deck, validator, content or prompt changes. |
| Demo-only Pages build + dark by default | shipped | on `github-pages-demo` — two changes to how the game is first met. **(1) The deployed build is demo-only.** New `client/src/staticDemo.js` exports one build-time constant, `STATIC_DEMO` (`VITE_STATIC_DEMO=1`, set by `.github/workflows/pages.yml` and nowhere else, so a local build is always the full game). On that build Begin starts the demo, and the Safe/Mature picker and the separate "Just show me" link are not rendered — a picker that cannot change anything and a second door into the room you are standing in are both worse than absent. It also skips the `/api/config` and `/api/region` probes, taking the deployed site from two guaranteed 404s per load to **no request of any kind** after the three assets. A build-time constant, NOT a runtime mode: it decides which entry point the start screen offers and reaches neither `state.demoMode`, the engine, nor the deck, and Begin still calls the same `beginDemo`, so the age gate still runs — verified with storage cleared. **(2) Dark is the default theme, app-wide.** `getActiveTheme()` returns the player's choice or `'dark'`, where it used to fall back to the OS's `prefers-color-scheme`; the choice still wins and still persists. The "follow the OS" third state is GONE from the toggle, because with dark as the default it rendered identically to `'dark'` and one press in three would have visibly done nothing. That change also exposed a latent bug worth naming: `toggleTheme` removed `.dark-theme` for any value that was not exactly `'dark'`, so the auto state already rendered light on a dark-preferring OS. Both copies of the class logic (App.jsx's mount effect, StartScreen's toggle) are now one `applyTheme()` in `prefs.js`. Verified in a browser on both builds: demo-only shows dark with no stored theme and runs 18/MATURE/"swipe 1 of 30" from Begin; the default build still shows the picker, "You start at 16" and the demo link; the toggle flips dark→light→dark with class, stored value and label agreeing every press. No engine, content, validator, deck or API-contract changes. |
| Demo mode published to GitHub Pages | shipped | on `github-pages-demo` — <https://kevinraffay.github.io/life-swipe/?demo=1>. A packaging job, not a feature: a demo life is ALREADY 100% static (the refill gate in `shared/deck.js`, `Obituary.jsx`'s skipped fetch and the absent intro phase remove every provider call; all content is a static import Vite inlines at build time), so the whole player client ships to a static host and demo mode is reached through the kiosk link it already had. Two changes only. `vite.config.js` gained `base: process.env.VITE_BASE || '/'` — env-driven and NOT hardcoded, because both cases are live at once: `npm start` serves `dist/` from Express at the domain root and needs '/', while a Pages project page lives under `/life-swipe/` where the default 404s every asset. `.github/workflows/pages.yml` runs `npm ci`, then `npm run demo-check` AS A GATE (the only automatic check of the demo's four assertions; a build failing them has no business reaching a public URL), then builds with `VITE_BASE=/life-swipe/` and publishes `dist/` via upload-pages-artifact/deploy-pages on every push to `main`, plus `workflow_dispatch` for deploying a branch by hand. VERIFIED IN A BROWSER against a real subpath rather than assumed: assets resolve, the kiosk link opens the age gate (never bypasses it), the demo runs at 18/MATURE/"swipe 1 of 30", and ten swipes made ZERO network requests. The two boot-time 404s (`/api/config`, `/api/region`) are expected and caught — "Storyteller offline" is the truth on a static host; they resolve to the github.io root rather than the project path since `client/src/api.js` uses absolute `/api/...`, which is harmless. A static-demo build flag (forcing demo as the bare-URL entry and skipping those two probes) was considered and deliberately NOT built: the URL already does the job. No engine, content, validator, client-component or API-contract changes; nothing an ordinary local life reads. |
...
`BAL.DEMO.maxSwipes` (30) is a ceiling, not a target. Bankruptcy and death end
...
thirtieth swipe ends as broke, not as "the demo finished".
...
The HUD shows `swipe 12 of 30` in demo mode. That one string is what stops the
cap reading as the game breaking: a player who can see the end coming
experiences it as the format.

**`BAL.DEMO.time` (5 months per minor swipe, against `BAL.TIME`'s 1) is a
...
meaningful chance of natural death inside thirty swipes needs roughly 1.5
years per swipe, which no minor card — "a moment or a week" — can carry without
the fiction collapsing. So the number is set for a different reason: at the
ordinary rate a 30-swipe demo covers two and a half years and ends at 20, which
reads as a fragment. At 5 months it covers ~13 years and ends around 31 —
college, first job, first real money — which is an arc, and which is why the
pool has three age bands reaching to 36 rather than one. The forced ending is
the normal demo ending **by design**, and is written for rather than
apologised for. Bankruptcy stays a genuine early exit and does fire.

**This constant and `maxSwipes` are one setting, not two.** Their product is
the age the demo reaches. When measurement pulled `maxSwipes` down, `time.minor`
had to go 4 → 5 with it: 30 × 4 months ends the demo at 28, which never enters
the pool's third age band and would strand its cards as unreachable. 30 × 5
ends at 30.5, which does — lightly, about one draw per demo, which is why that
band is the smallest share in `STAGE_SHARE`. Change one and re-check the other,
and read the age line `npm run demo-check` prints.
...
<https://kevinraffay.github.io/life-swipe/> — a **demo-only** build of the
player client on a static host. `.github/workflows/pages.yml` builds and
publishes it on every push to `main`.

**This is a packaging job, not a feature, and the reason it is possible is
that a demo life is already 100% static.** No server ever answers it: the
refill gate in `shared/deck.js`, `Obituary.jsx`'s skipped fetch and the absent
intro phase between them remove every provider call, and all content — the
demo pool, the name pool, the seed deck, the situation library — is a static
import Vite inlines into the JS at build time. Verified in a browser against a
real subpath, not assumed: after the three asset requests, ten swipes made
**zero** network requests.

**The deployed build is demo-only, and `client/src/staticDemo.js` is the whole
of that.** One build-time constant, `VITE_STATIC_DEMO=1`, set by the workflow
and by nothing else, so a local build is always the full game. On the Pages
build **Begin starts the demo**, and the Safe/Mature picker and the separate
"Just show me" link are not rendered — a picker that cannot change anything
and a second door into the room you are standing in are both worse than
absent. It also skips the `/api/config` and `/api/region` probes, since there
is nothing to answer them.

It is a build-time constant on purpose, **not a runtime mode**. It decides
which entry point the start screen offers and touches nothing else: not
`state.demoMode`, not the engine, not the deck. Begin still calls the same
`beginDemo`, so it still goes through the same age gate — verified with
storage cleared, not assumed. `?demo=1` still works; it is simply no longer
the way in.

Three things worth knowing before touching it:

- **`base` is env-driven, and has to be.** `npm start` serves `dist/` from
  Express at the domain root, so `'/'` stays the default; a Pages *project*
  page lives under `/life-swipe/`, where the default makes every asset 404.
  The workflow sets `VITE_BASE=/life-swipe/` and nothing else knows. Hardcode
  the subpath in `vite.config.js` and local play breaks instead. A repo
  rename, a custom domain or a user page (base `/`) is that one env line.
- **The two boot-time API probes are skipped**, not merely caught. They used
  to 404 on every load — harmlessly, since `getConfig()` failing reads as
  `llmEnabled: false` — but on a build that knows there is no server they were
  two guaranteed-dead requests, and they resolved to the github.io **root**
  rather than the project path, because `client/src/api.js` uses absolute
  `/api/...`. A demo session on the deployed build now makes **no** request of
  any kind after the three assets. Measured in a browser both before and
  after.
- **The local build is genuinely untouched by all of this**, and that was
  checked rather than reasoned about: built with no env vars, served at the
  root, and confirmed to show the Safe/Mature picker, "You start at 16" and
  the "Just show me" link, with Begin starting an ordinary life.

`npm run demo-check` runs in the workflow **before** the build, as a gate. It
is the only place the demo's four assertions get checked automatically, and a
build that fails its own assertions has no business reaching a public URL.

The admin cannot reach the artifact: `npm run build` never sees `admin/`, so
that exclusion is structural rather than something this workflow remembers.
